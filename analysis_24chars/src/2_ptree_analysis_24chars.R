...
#
# Three scenarios (same as full analysis):
# - Scenario A (Full): Train on entire period 1997-2022 
# - Scenario B (Split): Train 1997-2010, Test 2010-2022 
# - Scenario C (Reverse): Train 2010-2022, Test 1997-2010 
############################################################################
...
train_data_a <- data
results_a <- train_ptree_trio(train_data_a, "Full Sample (24 chars)")

output_dir_a = "results/ptree_24chars_scenario_a_full"
...
cat("Train: 1997-2010 | Test: 2010-2022\n")
cat(paste(rep("=", 80), collapse=""), "\n\n")
...
results_b <- train_ptree_trio(train_data_b, "Time Split (First Half, 24 chars)")
...
if (!is.null(factors_b_oos)) {
  ...
  cat("Saved OOS factors to:", file.path(output_dir_b, "ptree_factors_oos.csv"), "\n")
} else {
  cat("WARNING: OOS prediction failed for Scenario B\n")
}
cat("\n")
...
cat("Train: 2010-2022 | Test: 1997-2010\n")
cat(paste(rep("=", 80), collapse=""), "\n\n")

train_data_c <- data[data$date >= split_date, ]
test_data_c <- data[data$date < split_date, ]

results_c <- train_ptree_trio(train_data_c, "Reverse Split (Second Half, 24 chars)")
...
if (!is.null(factors_c_oos)) {
  ...
  cat("Saved OOS factors to:", file.path(output_dir_c, "ptree_factors_oos.csv"), "\n")
} else {
  cat("WARNING: OOS prediction failed for Scenario C\n")
}
cat("\n")
...
cat(paste(rep("=", 80), collapse=""), "\n")
cat("ANALYSIS COMPLETE - 24 MATCHED CHARACTERISTICS\n")
cat(paste(rep("=", 80), collapse=""), "\n\n")

t_total = proc.time() - t_total

summary_df = data.frame(
  Scenario = c("A: Full", "B: Split", "C: Reverse"),
  Train_Obs = c(nrow(train_data_a), nrow(train_data_b), nrow(train_data_c)),
  Test_Obs = c(0, nrow(test_data_b), nrow(test_data_c)),
  Tree1_Nodes = c(results_a$nodes[1], results_b$nodes[1], results_c$nodes[1]),
  Tree2_Nodes = c(results_a$nodes[2], results_b$nodes[2], results_c$nodes[2]),
  Tree3_Nodes = c(results_a$nodes[3], results_b$nodes[3], results_c$nodes[3]),
  Tree1_Sharpe = c(results_a$sharpes[1], results_b$sharpes[1], results_c$sharpes[1]),
  Tree2_Sharpe = c(results_a$sharpes[2], results_b$sharpes[2], results_c$sharpes[2]),
  Tree3_Sharpe = c(results_a$sharpes[3], results_b$sharpes[3], results_c$sharpes[3])
)

print(summary_df)
write.csv(summary_df, "results/ptree_24chars_all_scenarios_summary.csv", row.names = FALSE)

cat("\nTotal runtime:", round(t_total[3]/60, 1), "minutes\n")
cat("\nResults saved to results/ptree_24chars_scenario_*/\n")
cat("Summary saved to results/ptree_24chars_all_scenarios_summary.csv\n\n")
